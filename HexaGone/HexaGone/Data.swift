...
struct boardConfig {
    ...
    func boardWidth() -> CGFloat {
        return hexSize * (CGFloat(rows) + 1.5) // the +1.5 accounts for the alignment/shift factor
    }
    func boardHeight() -> CGFloat {
        return sqrt(3.0) / 2 * hexSize * CGFloat(cols)
    }
}

let beginnerBoard = boardConfig(rows: 10+4, cols: 11+4, mask: [
    ...
    Array(repeating: false, count: 2)+Array(repeating: true, count: 10)+Array(repeating: false, count: 3),
